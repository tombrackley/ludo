...
function useTextStream({ textStream, speed = 20, mode = 'typewriter', onComplete, fadeDuration, segmentDelay, characterChunkSize, onError }: UseTextStreamOptions): UseTextStreamResult {
    ...
    const updateSegments = useCallback((text: string) => {
        if (modeRef.current === 'fade') {
            try {
                ...
                type SegmenterConstructor = new (locales?: string | string[], options?: { granularity?: 'grapheme' | 'word' | 'sentence' }) => SegmenterLike

                const SegmenterCtor = (Intl as unknown as { Segmenter?: SegmenterConstructor }).Segmenter

                if (typeof SegmenterCtor === 'function') {
                    const segmenter = new SegmenterCtor(navigator.language, { granularity: 'word' })
